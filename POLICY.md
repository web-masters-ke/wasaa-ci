...
| N+1 detection              | Custom heuristics per ORM (Prisma, TypeORM, SQLAlchemy, Django, Tortoise, Drift, Isar, GORM, sqlx, database/sql). Each finding names the enclosing route handler (Express, NestJS, FastAPI, Flask, Django, Gin, Echo, Fiber, Chi) where detectable. | 0 findings in `for/forEach/map/range` loops calling ORM methods. Endpoint-scoped findings are P0. |
| Missing indexes (code-vs-schema) | `query_index_audit.py` — cross-references ORM schemas (Prisma, TypeORM, SQLAlchemy, Django, GORM) and Postgres migrations against every application query site. Flags columns filtered in code with no matching index. | Advisory (MEDIUM), non-blocking during initial rollout. Promotion to blocking follows the one-release advisory cycle. |
